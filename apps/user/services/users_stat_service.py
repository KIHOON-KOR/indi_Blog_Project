...
GRADE_SETTINGS = [
    {
        "min": 500,
        "label": "울창한 생명의 숲",
        "msg": "수많은 이야기가 모여 전설적인 숲을 이루었습니다!",
        "level": 9,
    ...
    },
    {
        "min": 30,
        "label": "튼튼한 묘목",
        "msg": "비바람에도 흔들리지 않는 튼튼한 줄기가 생겼어요.",
        "level": 3,
...
def get_user_garden_stats(user):
    ...
    current_grade = next(
        (g for g in GRADE_SETTINGS if total_count >= g["min"]), GRADE_SETTINGS[-1]
    )

    # 5. 다음 등급 계산: GRADE_SETTINGS를 역순으로 순회하여 남은 목표 등급 찾기
    reversed_settings = list(reversed(GRADE_SETTINGS))
    next_grade = next((g for g in reversed_settings if g["min"] > total_count), None)
